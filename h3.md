...
4. Comparison with Task B
Task B (XOR): The password was hidden by mathematically changing each letter. Even if you "unpacked" it, you still wouldn't see the word "sala" without the XOR key.

Task C (Packed): The password was hidden by compression. Once the file is "opened" by the computer to run, the password piilos-An exists in memory as plain text.
